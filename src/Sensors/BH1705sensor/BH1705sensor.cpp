...
BH1705sensor::BH1705sensor(TwoWire* wire, uint8_t i2cChannel, uint8_t tcaPort, float threshold, std::map<String, String> channels, int deviceIndex)
    : Device(wire, threshold, channels, i2cChannel, tcaPort, deviceIndex), _address(BH1705_ADDRESS), _lux(NAN) {
...
bool BH1705sensor::isConnected() {
    I2CHandler::selectTCA(getTCAChannel());    wire->beginTransmission(_address);
    return (wire->endTransmission() == 0);
}
